...
- 신경망의 학습과 추론에 드는 연산량은 상당합니다. 따라서 신경망을 얼마나 빠르게 계산하느냐는 매우 중요한 주제입니다.

  - 이번 절에서는 신경망 고속화에 도움되는 '비트 정밀도'와 'GPU'에 대해 가볍게 설명하겠습니다.

#### Note

> 이 책은 속도보다 알기 쉽게 구현하기에 우선순위를 두었습니다.
>
> 다만 고속화 관점에서 앞으로는 데이터의 비트 정밀도를 의식해 구현합니다.
>
> 또한 계산이 오래 걸리는 부분에서는 (선택적으로) GPU로 실행할 수 있도록 준비했습니다.

### 1.5.1 비트 정밀도

- NumPy의 부동소수점 수는 기본적으로 64비트 데이터 타입을 사용합니다.(이 환경은 사용자의 OS, python, NumPy 버전 등에 따라 다를 수 있습니다.)
  ...
  - 인스턴스 변수 dtype는 데이터 타입을 알아볼 수 있으며 float64의 경우 64비트 부동소수점 수라는 의미를 가집니다.

- NumPy는 64비트 부동소수점 수를 표준으로 사용하나 신경망의 추론과 학습은 32비트로도 문제없이 이를 수행할 수 있다고 합니다.

  - 32비트는 64비트의 절반으로 메모리 관점에서는 32비트가 더 유리합니다.

  - 또한 신경망 계산 과정에서 데이터를 전송하는 버스 대역폭에 병목현상이 생기는 경우가 있는데 이를 해결하기에도 32비트가 더 유리합니다.

  - 계산 속도 측면에서도 32비트가 더 유리합니다.(단, 이는 CPU나 GPU 아키텍처에 따라 다릅니다.)

- 이런 이유로 이 책에서는 32비트 부동소수점 수를 우선으로 사용하는데 이를 NumPy에서 지정하는 방법은 다음과 같습니다.
...
- 또한 신경망 추론에서는 16비트 부동소수점 수를 사용해도 인식률이 거의 떨어지지 않습니다.(최근엔 8비트 계산도 가능합니다.)

  - NumPy 역시 이를 지원하나 CPUㅏ와 GPU는 일반적으로 32비트를 기준으로 연산을 진행합니다. 따라서 16비트로 전환하더라도 처리 속도의 이득이 없을 수 있습니다.

  - 하지만 학습된 가중치를 저장할 때는 16비트 부동소수점 수가 유효하는데 이는 32비트의 절반의 용량을 차지하기 때문입니다.

    - 따라서 이 책에서 학습된 가중치를 저장할 때는 16비트 부동소수점 수로 할 계획입니다.

#### Note

> 딥러닝이 주목받으며 최근 GPU들은 저장과 연산 모두 16비트 반정밀도 부동소수점 수를 지원하며 구글의 TPU는 8비트 연산도 지원합니다.

### 1.5.2 GPU with CuPy

- 딥러닝 계산은 대량의 곱셈으로 대부분 병렬로 계산할 수 있다는 점에서 CPU보다 GPU를 사용하는 게 더 유리합니다.

  - 따라서 이 책의 예제 중에서 CuPy를 사용할 수 있는 게 있습니다.

- CuPy는 GPU를 이용해 병렬 계산이 가능하도록 해주는 라이브러리로 엔비디아의 GPU만 가능하며 CUDA라는 GPU 범용 병렬 컴퓨팅 플랫폼의 설치가 필요합니다.

  > 자세한 내용은 [CuPy 공식 설치 가이드](https://docs-cupy.chainer.org/en/stable/install.html#install-cupy)를 참고하세요.

- CuPy는 엔비디아 GPU를 사용해 간단한 병렬 계산이 가능하며 NumPy와 호환되는 API를 제공합니다. 그 예시는 다음과 같습니다.
...
- 이처럼 CuPy의 사용법은 일반적으로 NumPy와 같지만 GPU를 사용해 계산한다는 차이가 있습니다.

#### Warning

> CuPy가 모든 NumPy 메서드를 지원하는 건 아니므로 API를 사용할 때 주의가 필요합니다.

- 다만 이 책은 이해하기 쉽게 구현하는 걸 우선하므로 기본적으로 CPU에서 수행되는 코드를 작성합니다.

  - 그러나 계산이 오래 걸리는 코드는 선택적으로 CuPy를 사용할 수 있도록 구현되어 있습니다.

  - 또한 CuPy를 사용할지 말지는 사용자의 몫으로 남겨져 있으므로 본인의 편의를 따르시길 바랍니다.

  > 일반적으로 config.GPU = True로 되어있는 부분을 주석해제하는 것으로 CuPy의 사용이 가능합니다.

#### Note

> 더 자세한 CuPy의 동작 방식은 chapter01/commons의 config.py, np.py, layers.py의 import 문을 확인해주세요.

## 1.6 정리

- 신경망은 입력층, 은닉층, 출력층을 가집니다.

- 완전연결계층에 의해 선형 변환이 이루어지고 활성화 함수에 의해 비선형 변환이 이루어집니다.

- 완전연결계층이나 미니배치 처리는 행렬로 모아 한 번에 처리가 가능합니다.

- 오차역전파법을 사용해 신경망의 손실에 관한 기울기를 효율적으로 구할 수 있습니다.

- 신경망이 수행하는 처리는 계산 그래프로 시각화할 수 있으며, 순전파와 역전파를 이해하는 데 도움이 됩니다.

- 신경망의 구성요소들을 계층으로 모듈화하면 이를 조립하여 신경망을 쉽게 구현할 수 있습니다.

- 신경망 고속화에는 GPU를 이용한 병렬 계산과 데이터의 비트 정밀도가 중요합니다.

...
- 추론 기반 기법은 추측하는 것이 목적이며, 그 부산물로 단어의 분산 표현을 얻을 수 있습니다.

- word2vec은 추론 기반 기법으로 단순한 2층 신경망입니다.

- word2vec은 skip-gram 모델과 CBOW 모델을 제공합니다.

- CBOW 모델은 여러 단어(맥락)에서 하나의 단어(타깃)을 추측합니다.

- 반대로 skip-gram 모델은 하나의 단어(타깃)에서 다수의 단어(맥락)을 추측합니다.

- word2vec은 가중치를 다시 학습할 수 있으므로, 단어의 분산 표현 갱신이나 새로운 단어 추가를 효율적으로 수행할 수 있습니다.
